        ...
        <ul class="nav nav-tabs primary">
            ...
            <div class="box-tools pull-right">
                ...
                <button type="button" class="btn btn-primary btn-sm" data-action="EDIT" data-load-to='#user-client-entry' data-href='{{ guard_url('user/' . $type) }}/{{$client->getRouteKey()}}/edit'><i class="fa fa-pencil-square"></i> {{ trans('app.edit') }}</button>
                    @if($client->expired)
                        <button type="button" class="btn btn-warning btn-sm" data-action="RELEASE" data-load-to='#user-client-entry' data-id='{{$client->id}}' href='#'><i class="fa fa-clock-o"></i> Expirar Login</button>
                    @else
                        <button type="button" class="btn btn-success btn-sm" data-action="EXPIRY" data-load-to='#user-client-entry' data-id='{{$client->id}}' href='#'><i class="fa fa-clock-o"></i> Liberar Login</button>
                    @endif
